...
| [ui/](ui/) | Streamlit web application (dashboard, data browser, schedule view, embedded docs). |
...
| **Tab · Dashboard** | Club picker (`_render_club_picker`): grid of buttons sets `st.session_state["dashboard_club"]`. **Club season** table (`_club_season_table`): all rows from `_schedule_dataframe()` where the club is home or away, with `H_A` and `Opponent` columns; sorted by `Date_time`. Schedule source: last successful `OptimizationResult.schedule_df`, else `output/optimized_schedule.csv`. Then simulation metrics and expanders. |
